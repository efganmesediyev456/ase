...
$(document).ready(function () {
    ...
    $(document).on("keypress", function (e) {
        if (e.shiftKey && (e.which == 13)) {
            if ($("#new_package").length > 0) {
                $("#new_package input[name='tracking_code']").val("");
                $("#new_package input[name='weight']").val("");
                $("#new_package input[name='ru_weights[]']").val("");
                $("#alert_weight").hide();
                $("#alert_price").hide();
    ...
    $("#add_key").on("click", function () {
        var clone = $("#main_key_item").clone();
        clone.removeAttr("id");
        clone.addClass("extra_type");
	clone.find('input[name=keys\\[\\]]').val('');
	clone.find('select[name=values\\[\\]]').val('ADDRESS');
    ...
    $("select[name='invoice_type']").on("change", function () {
        var w_val = $("select[name='invoice_type']").children(":selected").attr("value");
        if (w_val == 1) {
 ...
 $("#brHeader").change(function (event) {
   let brcode=this.value;
   if(brcode=='') return;
   this.value='';
   if(brcode.length<5) return;
   loadBarcodeData(brcode, 1);
 });
...
function loadBarcodeData(barcode, qty) {
    ...
    var bagId = 0;
    var checkDraftLabel = $("#check_draft_label").is(':checked');
    ...
    if(scanUrl.charAt(0) == '/') scanUrl=scanUrl.substring(1);
    _url+='&path='+scanUrl;

    var courierId = $("[name='courier_id']").val();
    var courierTracksScanUrl=$("#courier_tracks_scan_url").data('scan-url');
	console.log(courierTracksScanUrl);
    if((courierId !== undefined) && (courierTracksScanUrl !== undefined)) {
        _url = courierTracksScanUrl + "/" + barcode + "?courier_id=" + courierId;
    }
    ...
    $.getJSON(_url, function (data) {
        if (data.error || data.warning) {
	    var audio = new Audio('/sounds/scan_alert.mp3');
	    audio.play();
            if (data.new_system) {
                swal({
                    "timer": 3000,
                    "title": "<b>" + data.error + "</b>",
        ...
        } else if (data.add_track) {
              if ($("#package_" + data.track.id).length == 0) {
                $("#empty_package").hide();
                $("#manual_add").val("");
                scannerTab.append(data.html);
		$("#tracks_count").html(scannerTab[0].childElementCount);
	      }
            ...
            if ((data.is_ready || checkDraftLabel) && data.label) {
                //if (autoPrint == 'yes') {
                if (autoPrint && ((bagId > 1 && autoPrintParcel == 'yes') || (bagId <= 1 && autoPrintPackage == 'yes'))) {
                    ...
                    let d_url = data.label + '?cdfl=';
                    if (checkDraftLabel)
                        d_url = d_url + '1';
                    else
                        d_url = d_url + '0';
                    var labelPDF = new JSPM.PrintFilePDF(d_url, JSPM.FileSourceType.URL, 'MyLabelFile.pdf', copies);
