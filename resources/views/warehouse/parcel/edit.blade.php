...
@section('content')
    <div class="row">
        {!! Form::open(['route' => [$crud['route'] . '.update', $bag->id], 'method' => 'put', 'id' => 'package_ids']) !!}
...
<br>
                            <div>
                                ...
                                <div class="input-group">
                                    <div class="input-group-addon"><i class="icon-barcode2"></i></div>
                                    <input id="bname" type="text" name="bname" placeholder="(Optional) Leave empty for autonumbering" value="{{ $bag->custom_id }}"
                                           class="form-control">
                                </div>
                ...
                <div class="panel-body">
                    <div class="table-responsive overflow-visible">
                        <table class="table table-hover responsive table-striped">
                            ...
                            <tbody id="scanned" data-id="{{ $bag->id }}">
                            @foreach($bag->packages as $citem)
                ...
                <div class="panel-heading" style="height: 150px; border-top: 1px solid #ccc;">
                    <div class="heading-delements">
                        ...
                        <div class="col-md-4">
                            @if ($parcel->status == 0 && ! auth()->guard('worker')->user()->warehouse->allow_make_fake_invoice)
                                <span style="position: relative; top: 35px;margin-right: 20px; border: 1px solid #ff1c90; padding: 10px; border-radius: 5px">Waiting ready packages : <a
                                            href="{{ route('w-packages.index') }}?status=0&dec=1&limit=25&incustoms=2"
