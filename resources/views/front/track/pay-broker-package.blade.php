                                ...
                                <div class="col-lg-12">
                                    <div class="col-md-offset-2 col-md-8 col-sm-12 col-xs-12">
                                        ...
                                        @if($item->paid_broker == 0)
                                            <div class="row" id="pay">
                                                <div class="col-sm-12 col-lg-8">
                                                    <h3>{{__('front.pay_title_broker',['track'=>$item->custom_id])}}</h3>
                                                </div>
                                            </div>
                                            <div class="row">
                                                <div class="col-md-12 col-sm-12 col-xs-12" style="width:100%">
                                                    <form action="{{ route('kapital.new.payment', 'package_broker') }}" method="POST" style="display: flex">
                                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                        <input type="hidden" name="item_id" value="{{ $item->id }}">
                                                        <input type="hidden" name="amount" value="{{(empty($item->user->voen)) ? 15 : 50}}">
                                                        ...
                                                        <h3 class="repayment" style=" padding: 10px 15px 10px 5px" id="pkg_amount_lbl">
                                                            {{(empty($item->user->voen)) ? 15 : 50}} AZN</h3>
                                        ...
                                        @else
                                            <div class="row" id="pay">
                                                <div class="col-sm-12 col-lg-8">
                                                    <h3>{{__('front.pay_not_found_debt',['track'=>$item->custom_id])}}</h3>
                                                </div>
                                            </div>
                                        @endif
                                    </div>
