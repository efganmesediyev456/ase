...
$items = old($field['name']) ? (old($field['name'])) : (isset($item->{$field['name']}) ? ($item->{$field['name']}) : (isset($field['default']) ? ($field['default']) : ''));
...
<div ng-app="backPackTableApp" ng-controller="tableController" @include('crud::inc.field_wrapper_attributes') >
    ...
    <div class="array-container form-group">
        ...
        <table class="table table-bordered table-striped m-b-0"
               ng-init="field = '#{{ $field['name'] }}'; items = {{ $items }}; max = {{$max}}; min = {{$min}}; maxErrorTitle = '{{trans('saysay::crud.table_cant_add', ['entity' => $item_name])}}'; maxErrorMessage = '{{trans('saysay::crud.table_max_reached', ['max' => $max])}}'">
        ...
        <div class="array-controls btn-group m-t-10">
            <button ng-if="max == -1 || items.length < max" class="btn btn-sm btn-default" type="button"
                    ng-click="addItem()"><i class="fa fa-plus"></i> {{trans('saysay::crud.add')}} {{ $item_name }}
            </button>
        </div>
