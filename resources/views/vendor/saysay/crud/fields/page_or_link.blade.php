...
<?php
$field['options'] = [
    'page_link' => trans('saysay::crud.page_link'),
    'internal_link' => trans('saysay::crud.internal_link'),
    'external_link' => trans('saysay::crud.external_link')
];
        ...
        >
            ...
            @if (count($field['options']))
                @foreach ($field['options'] as $key => $value)
                    <option value="{{ $key }}"
                            @if (isset($item->{$field['name']}) && $key==$item->{$field['name']})
                            selected
                            @endif
                    >{{ $value }}</option>
                @endforeach
    ...
    <div class="col-sm-9">
        ...
        <div class="page_or_link_value <?php if (! isset($entry) || $entry->type != 'external_link') {
            ...
            <input
                    ...
                    class="form-control"
                    name="link"
                    placeholder="{{ trans('saysay::crud.page_link_placeholder') }}"
        ...
        <div class="page_or_link_value <?php if (! isset($entry) || $entry->type != 'internal_link') {
            ...
            <input
                    ...
                    class="form-control"
                    name="link"
                    placeholder="{{ trans('saysay::crud.internal_link_placeholder', ['url', url(config('backpack.base.route_prefix').'/page')]) }}"
