...
<div class="form-group col-md-12 checklist_dependency"
    ...
    if (isset($id) && $id) {
        ...
        $entity_dependencies = $entity_model->with($primary_dependency['entity'])->with($primary_dependency['entity'] . '.' . $primary_dependency['entity_secondary'])->find($id);
        ...
        <div class="hidden_fields_primary" data-name="{{ $primary_dependency['name'] }}">
            @if(isset($item->{$field['name']}))
                @if(old($primary_dependency['name']))
                    ...
                    @endforeach
                @else
                    @foreach( $item->{$field['name']}[0]->pluck('id', 'id')->toArray() as $item )
                        <input type="hidden" class="primary_hidden" name="{{ $primary_dependency['name'] }}[]"
                               value="{{ $item }}">
                    @endforeach
                @endif
            ...
            <div class="col-sm-{{ isset($primary_dependency['number_columns']) ? intval(12/$primary_dependency['number_columns']) : '4'}}">
                <div class="checkbox">
                    <label>
                        ...
                        @if( ( isset($item->{$field['name']}) && is_array($item->{$field['name']}) && in_array($connected_entity_entry->id, $item->{$field['name']}[0]->pluck('id', 'id')->toArray())) || ( old($primary_dependency["name"]) && in_array($connected_entity_entry->id, old( $primary_dependency["name"])) ) )
                            checked = "checked"
                        @endif >
        ...
        <div class="hidden_fields_secondary" data-name="{{ $secondary_dependency['name'] }}">
            @if(isset($item->{$field['name']}))
                @if(old($secondary_dependency['name']))
                    ...
                    @endforeach
                @else
                    @foreach( $item->{$field['name']}[1]->pluck('id', 'id')->toArray() as $item )
                        <input type="hidden" class="secondary_hidden" name="{{ $secondary_dependency['name'] }}[]"
                               value="{{ $item }}">
                    @endforeach
                @endif
            ...
            <div class="col-sm-{{ isset($secondary_dependency['number_columns']) ? intval(12/$secondary_dependency['number_columns']) : '4'}}">
                <div class="checkbox">
                    <label>
                        ...
                        @if( ( isset($item->{$field['name']}) && is_array($item->{$field['name']}) && (  in_array($connected_entity_entry->id, $item->{$field['name']}[1]->pluck('id', 'id')->toArray()) || isset( $secondary_ids[$connected_entity_entry->id])) || ( old($secondary_dependency['name']) &&   in_array($connected_entity_entry->id, old($secondary_dependency['name'])) )))
                            checked = "checked"
