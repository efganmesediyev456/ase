...
@section('content')
        ...
        <div class="card_body">
            <div class="box box-primary p-15">
                <div class="box-body">
                    <div class="row">
                        <div class="col-md-8">
                            <form method="post" action="{{ route('surat.containers') }}">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <div class="row">
                                    <div class="col-md-4">
                                        <div class="input-group">
                                            <input type="text" name="name" value="" class="form-control" placeholder="Ad" autofocus>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <select name="surat_office_id" class="form-control select2">
                    ...
                    <div class="row">
                        <form method="get" action="{{ route('surat.containers') }}">
                            <div class="col-md-4">
                                ...
                                <div class="form-group" style="display: flex;align-items: center">
                                    <select id="status_" name="status" class="form-control">
                                        ...
                                        @foreach($statuses as $value => $key)
                                            ...
                                            <option value="{{ $key }}"
                                                    @if(request()->filled('status') && request()->get('status') == $key) selected="selected" @endif>{{ __('admin.surat_warehouse_group_status_'.$key) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
            ...
            @if($rows->count() > 0)
                <div class="box box-primary mt-15">
                    <div class="box-body table-responsive no-padding">
                        <form method="post" id="itemsForm" action="{{ route('surat.index') }}">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                    ...
                                    <tr>
                                        <td class="text-right">
                                            <div style="display: flex">
                                                <a href="{{route('surat.containers.edit', $row->id)}}"
                                        ...
                                        <td>{{ __('admin.surat_warehouse_group_status_'.$row->status) }}</td>
                                        ...
                                        <td>{{ $row->sent_at }}</td>
                                        <td>
                                            <a href="{{ route('containerSend', ['id' => $row->id, 'type' => 'surat']) }}"
                                               class="btn btn-success" type="button">
                                                <span class="text-white">Göndər</span>
                                            </a>
                                            <a href="{{ route('containerCheck', ['id' => $row->id, 'type' => 'surat']) }}"
                                            ...
                                            @if($row->status == \App\Models\Surat\SuratOrder::STATUSES['WAITING'])
                                                <button
                                                        formaction="{{ route('surat.delete_group', $row->id) }}"
    ...
    <div class="modal edit-modal fade" id="containerEditModal" role="dialog"
        ...
        <div class="modal-dialog modal-sm" role="document">
            <form action="{{ route('surat.update_name') }}" method="post">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            ...
                            <div class="form-group">
                                <select name="status" class="form-control">
                                    ...
                                    @foreach($statuses as $status => $key)
                                        <option value="{{ $key }}"> {{ __('admin.surat_warehouse_group_status_'.$key) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
