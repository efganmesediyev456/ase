...
@section('content')
        ...
        <div class="card_body">
            <div class="box box-primary p-15">
                <div class="box-body">
                    <div class="row">
                        <div class="col-md-8">
                            <form method="post" action="{{ route('kargomat.containers') }}">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <select name="azeri_express_office_id" class="form-control select2">
                    ...
                    <div class="row">
                        <form method="get" action="{{ route('kargomat.containers') }}">
                            <div class="col-md-4">
                                ...
                                <div class="form-group" style="display: flex;align-items: center">
                                    <select id="status_" name="status" class="form-control">
                                        ...
                                        @foreach($statuses as $value => $key)
                                            ...
                                            <option value="{{ $key }}"
                                                    @if(request()->filled('status') && request()->get('status') == $key) selected="selected" @endif>{{ __('admin.kargomat_warehouse_group_status_'.$key) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
            ...
            @if($rows->count() > 0)
                <div class="box box-primary mt-15">
                    <div class="box-body table-responsive no-padding">
                        <form method="post" id="itemsForm" action="{{ route('kargomat.index') }}">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                ...
                                @foreach($rows as $row)
                                    ...
                                    <tr>
                                        <td style="display: flex; justify-content: space-evenly; gap: 5px">
                                            <a href="{{ route('containerCheck', ['id' => $row->id, 'type' => 'kargomat']) }}"
                                               class="btn btn-primary" type="button">
                                                <span class="text-white">Check</span>
                                            </a>
                                            <div style="display: flex">
                                                <a href="{{route('kargomat.containers.edit', $row->id)}}"
                                        ...
                                        <td>{{ __('admin.kargomat_warehouse_group_status_'.$row->status) }}</td>
                                        ...
                                        <td>{{ $row->sent_at }}</td>
                                        <td>
                                            <a href="{{ route('containerSend', ['id' => $row->id, 'type' => 'kargomat']) }}"
                                            ...
                                            <button
                                                    formaction="{{ route('kargomat.delete_group', $row->id) }}"
    ...
    <div class="modal edit-modal fade" id="containerEditModal" role="dialog"
        ...
        <div class="modal-dialog modal-sm" role="document">
            <form action="{{ route('kargomat.update_name') }}" method="post">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            ...
                            <div class="form-group">
                                <select name="status" class="form-control">
                                    ...
                                    @foreach($statuses as $status => $key)
                                        <option value="{{ $key }}"> {{ __('admin.kargomat_warehouse_group_status_'.$key) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
