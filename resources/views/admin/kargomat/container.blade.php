...
@section('title', 'Kargomat - Konteyner')
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <br>
                    <b>Konteyner Status: </b>{{ __('admin.kargomat_warehouse_group_status_'.$container->status) }}
                    <br>

                    <a href="{{ route('kargomat.containers.edit', $container->id) }}">
                        Ümumi Bağlama sayı: {{$packagesCount ?? ''}}
                    </a>
                    <br>
                    <a href="{{ route('kargomat.containers.edit', $container->id) }}" class="text-success">
                        Göndərilən Bağlama sayı: {{$sentPackagesCount ?? ''}}
                    </a>
                    <br>
                    <a href="{{ route('kargomat.containers.edit', $container->id) }}" class="text-warning">
                        Göndərilməyən Bağlama sayı: {{$notSentPackagesCount ?? ''}}
                    </a>
                    <br>
                    <a href="{{ route('kargomat.containers.edit', $container->id) }}?status=2" class="text-danger">
                ...
                <div class="col-md-6 text-right">
                    ...
                    <form method="post"
                          action="{{ route('kargomat.send-packages', $container->id) }}"
                        ...
                        {{--                            Keşi sil--}}
                        {{--                        </button>--}}
{{--                        @if($container->status === \App\Models\Kargomat\KargomatOrder::STATUSES['WAITING'])--}}

{{--                        --}}
{{--                        @endif--}}
                        <button class="btn btn-success"
                                data-url="{{ route('kargomat.send-packages', $container->id) }}"
                        >
                            Göndər
                        </button>
                        <button class="btn btn-success"
                                data-url="{{ route('kargomat.send-packages', $container->id) }}?temu=true"
                        >
                            Göndər Temu
                        </button>
                    </form>
    ...
    <div class="card" style="max-width: 300px">
        <div class="card-header">
            <form method="get"
                  action="{{ route('kargomat.containers.edit', $container->id) }}"
                  class="form-prevent-multiple-submits"
            >
    ...
    <div class="card box-primary">
        <div class="card-header">
            <form method="post"
                  action="{{ route('kargomat.store', $container->id) }}"
                        ...
                        @foreach($packages as $row)
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @endphp
                                    <span class="label label-{{ $class }}">{{ __('admin.kargomat_warehouse_package_status_'.$row->status) }}</span>
                                </td>
                                <td>
                                    @php
                                        $_type = "package";
                                ...
                                <td>
                                    <button formaction="{{ route('kargomat.delete-package', $row->id) }}"
