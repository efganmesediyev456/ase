...
@section('content')
        ...
        <div class="col-lg-{{ $_view['listColumns'] }} col-lg-offset-{{ (12 - $_view['listColumns'])/2 }} col-md-12 col-xs-12">
		   ...
		   <?php $noNeed=true; ?>
                    <div class="table-responsive">
                        <table class="table table-hover responsive table-bordered">
                            ...
                            ?>
                                ...
                                <tr>
                                    <td class="tab_it closed" data-bags="parcel_{{ $ctId }}" data-packages="parcel2_{{ $ctId }}" data-tabs="tab_{{ $ctId }}" }}>
					...
					{{$ctName}}
                                    ...
                                    <td colspan="2"></td>
                                    <td>{{ $item->tracks->sum('weight') }} kg / {{ $item->tracks->sum('declared_weight_goods') }} kg</td>
				    <td>	{{ $item->tracks_count }}  @if($item->track_not_completed_count) /<a
                                                    href="{{ route('tracks.index') }}?dec=2&partner_id={{ $item->partner_id }}&limit=25&parcel={{ $item->name }}"
                                                    class="waiting_packages"
                                                    style="background: red;color: #fff;padding: 5px;border-radius: 5px;">{{ $item->track_not_completed_count }}</a>@endif
                                    </td>
				    <td>
				    		{{ $item->tracks_count }} /
                ...
                <div class="modal-body">
                    <form id="updateContainerForm" action="" method="POST">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="containerName" name="containerName" placeholder="Enter new container name">
                        </div>
                    </form>
                </div>
    ...
    <div class="modal fade" id="updateMawb" tabindex="-1" role="dialog" aria-labelledby="updateModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="updateModalLabel">Update Container Name</h5>
                ...
                <div class="modal-body">
                    <form id="updateMawbForm" action="" method="POST">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="bagName" name="bagName" placeholder="Enter new bag name">
                        </div>
                        <div class="form-group">
                            <label for="trackings">Tracks</label>
                            <textarea class="form-control" id="trackings" name="trackings" rows="4" placeholder="Enter new trackings"></textarea>
                        </div>
                    </form>
                </div>
                <div class="modal-footer">
