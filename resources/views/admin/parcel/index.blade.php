                    ...
                    <div class="table-responsive">
                        <table class="table table-hover responsive table-bordered">
                            ...
                            @forelse($items as $row => $item)
                                ...
                                <?php $parcelCustomId = $item->custom_id; ?>
                                <tr>
                                    <td class="tab_it closed" data-bags="parcel_{{ $parcelId }}" data-packages="parcel2_{{ $parcelId }}" data-tabs="tab_{{ $parcelId }}" }}>
                                        ...
                                        @include('crud::components.columns.editable', [
                                            'entry' => parseRelation($item, 'custom_id'),
                                            'head' => [
                                                        'type'     => 'editable',
                                                        'editable' => [
                                                            'title' => 'MAWB',
                                    ...
                                    <td colspan="{{  intval(isset($_view['checklist'])) + count($_list) - 7 }}"></td>
                                    <td>{{ $item->packages->sum('weight_goods') }} kg</td>
                                    <td>
                                        {{ $item->packages_count }}
                                        @if(!$item->sent && $item->ready_cnt)
                                            /
                                            <a
                                                    href="{{ route('packages.index') }}?status=0&warehouse_id={{ $item->warehouse_id }}&limit=25&dec=2&incustoms=2"
                                                    class="waiting_packages"
                                                    style="background: #f48399;color: #fff;padding: 5px;border-radius: 5px;">{{ $item->ready_cnt }}</a>
                                        @endif
                                        @if($item->waiting_count)
                                            /<a
                                                    href="{{ route('packages.index') }}?status=1&warehouse_id={{ $item->warehouse_id }}&limit=25&parcel={{ $parcelCustomId }}"
                                                    class="waiting_packages"
                                                    style="background: red;color: #fff;padding: 5px;border-radius: 5px;">{{ $item->waiting_count }}</a>
                                        @endif
                                    </td>
                                    <td>
                                        {{ $item->packages_count }} /
				...
				<tr class="sub-child  parcel_{{ $parcelId }}">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                    ...
                                    <td>{{ $item->packages_count }} @if($item->waiting_count) /<a
                                                href="{{ route('packages.index') }}?status=1&warehouse_id={{ $item->warehouse_id }}&limit=25&parcel={{ $parcelCustomId }}&bag={{ $item->custom_id }}"
                                                class="waiting_packages"
