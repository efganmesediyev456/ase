...
@section('content')
        ...
        <div class="card_body">
            <div class="box box-primary p-15">
                <div class="box-body">
                    <div class="row">
                        <div class="col-md-8">
                            <form method="post" action="{{ route('azerpost.containers') }}">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <select name="azerpost_office_id" class="form-control select2">
                    ...
                    <div class="row">
                        <form method="get" action="{{ route('azerpost.containers') }}">
                            <div class="col-md-4">
                                ...
                                <div class="form-group" style="display: flex;align-items: center">
                                    <select id="status_" name="status" class="form-control">
                                        ...
                                        @foreach($statuses as $value => $key)
                                            ...
                                            <option value="{{ $key }}"
                                                    @if(request()->filled('status') && request()->get('status') == $key) selected="selected" @endif>{{ __('admin.azerpost_warehouse_group_status_'.$key) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
            ...
            @if($rows->count() > 0)
                <div class="box box-primary mt-15">
                    <div class="box-body table-responsive no-padding">
                        <form method="post" id="itemsForm" action="{{ route('azerpost.index') }}">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                ...
                                @foreach($rows as $row)
                                    ...
                                    <tr>
                                        <td style="display: flex; justify-content: space-evenly; gap: 5px">
                                            <a href="{{ route('containerCheck', ['id' => $row->id, 'type' => 'azerpost']) }}"
                                               class="btn btn-primary" type="button">
                                                <span class="text-white">Check</span>
                                            </a>
                                            <div style="display: flex">
                                                <a href="{{route('azerpost.containers.edit', $row->id)}}"
                                        ...
                                        <td>{{ __('admin.azerpost_warehouse_group_status_'.$row->status) }}</td>
                                        ...
                                        <td>{{ $row->sent_at }}</td>
                                        <td>
                                            <a href="{{ route('containerSend', ['id' => $row->id, 'type' => 'azerpost']) }}"
                                            ...
                                            <button
                                                    formaction="{{ route('azerpost.delete_group', $row->id) }}"
    ...
    <div class="modal edit-modal fade" id="editContainerModal" role="dialog"
         aria-labelledby="editDeclarationPriceFromListModal">
        <div class="modal-dialog modal-sm" role="document">
            <form action="{{ route('azerpost.update_name') }}" method="post">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            ...
                            <div class="form-group">
                                <select name="status" class="form-control">
                                    ...
                                    @foreach($statuses as $status => $key)
                                        <option value="{{ $key }}"> {{ __('admin.azerpost_warehouse_group_status_'.$key) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
