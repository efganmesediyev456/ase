...
@section('title', 'Azerpost - Konteyner')
...
@section('content')
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <br>
                    <b>Konteyner Status: </b>{{ __('admin.azerpost_warehouse_group_status_'.$group->status) }}
                    <br>

                    <a href="{{ route('azerpost.containers.edit', $group->id) }}">
                        Ümumi Bağlama sayı: {{$packagesCount ?? ''}}
                    </a>
                    <br>
                    <a href="{{ route('azerpost.containers.edit', $group->id) }}" class="text-success">
                        Göndərilən Bağlama sayı: {{$sentPackagesCount ?? ''}}
                    </a>
                    <br>
                    <a href="{{ route('azerpost.containers.edit', $group->id) }}" class="text-warning">
                        Göndərilməyən Bağlama sayı: {{$notSentPackagesCount ?? ''}}
                    </a>
                    <br>
                    <a href="{{ route('azerpost.containers.edit', $group->id) }}?status=2" class="text-danger">
                ...
                <div class="col-md-6 text-right">
                    ...
                    <form method="post"
                          action="{{ route('azerpost.send-packages', $group->id) }}"
                        ...
                        <button class="btn btn-success"
                                data-url="{{ route('azerpost.send-packages', $group->id) }}?temu=true"
                        >
                            Göndər Temu
                        </button>
                    </form>
    ...
    <div class="card" style="max-width: 300px">
        <div class="card-header">
            <form method="get"
                  action="{{ route('azerpost.containers.edit', $group->id) }}"
                  class="form-prevent-multiple-submits"
            >
    ...
    <div class="card box-primary">
        <div class="card-header">
            <form method="post"
                  action="{{ route('azerpost.store', $group->id) }}"
                        ...
                        @foreach($packages as $row)
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @endphp
                                    <span class="label label-{{ $class }}">{{ __('admin.azerpost_warehouse_package_status_'.$row->status) }}</span>
                                </td>
                                <td>
                                    @php
                                        $_type = "package";
...
//                                        $_status = $row->type == 'track'?$row->track->status:$row->package->status;
                                ...
                                <td>
                                    <button formaction="{{ route('azerpost.delete-package', $row->id) }}"
