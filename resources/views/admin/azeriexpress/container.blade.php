        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <br>
                    <b>Konteyner Status: </b>{{ __('admin.azeriexpress_warehouse_group_status_'.$container->status) }}
                    <br>

                    <a href="{{ route('azeriexpress.containers.edit', $container->id) }}">
                        Ümumi Bağlama sayı: {{$packagesCount ?? ''}}
                    </a>
                    <br>
                    <a href="{{ route('azeriexpress.containers.edit', $container->id) }}" class="text-success">
                        Göndərilən Bağlama sayı: {{$sentPackagesCount ?? ''}}
                    </a>
                    <br>
                    <a href="{{ route('azeriexpress.containers.edit', $container->id) }}" class="text-warning">
                        Göndərilməyən Bağlama sayı: {{$notSentPackagesCount ?? ''}}
                    </a>
                    <br>
                    <a href="{{ route('azeriexpress.containers.edit', $container->id) }}?status=2" class="text-danger">
                ...
                <div class="col-md-6 text-right">
                    ...
                    <form method="post"
                          action="{{ route('azeriexpress.send-packages', $container->id) }}"
                        ...
                        {{--                            Keşi sil--}}
                        {{--                        </button>--}}
{{--                        @if($container->status === \App\Models\AzeriExpress\AzeriExpressOrder::STATUSES['WAITING'])--}}
{{--                            --}}
{{--                        @endif--}}
                        <button class="btn btn-success"
                                data-url="{{ route('azeriexpress.send-packages', $container->id) }}"
                        >
                            Göndər
                        </button>
                        <button class="btn btn-success"
                                data-url="{{ route('azeriexpress.send-packages', $container->id) }}?temu=true"
                        >
                            Göndər Temu
                        </button>
                    </form>
    ...
    <div class="card" style="max-width: 300px">
        <div class="card-header">
            <form method="get"
                  action="{{ route('azeriexpress.containers.edit', $container->id) }}"
                  class="form-prevent-multiple-submits"
            >
    ...
    <div class="card box-primary">
        <div class="card-header">
            <form method="post"
                  action="{{ route('azeriexpress.store', $container->id) }}"
                    ...
                    <table class="table table-hover table-striped">
                        ...
                        @foreach($packages as $row)
                            ...
                            <tr>
                                ...
                                <td>
                                    <button formaction="{{ route('azeriexpress.delete-package', $row->id) }}"
