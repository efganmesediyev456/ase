...
class MainController extends Controller
{
    ...
    {
        //return redirect()->route('courier_deliveries.index');
    ...
    public function barcodeScan($code)
    {
        try {
            ...
            if ($item) {
                ...
                if ($bag_id <= 1 && auth()->guard('worker')->user()->warehouse->only_weight_input && !$item->weight_goods) {
                    return response()->json([
                        'add_package' => true,
                ...
                if (!($checkDraftLabel && $bag_id <= 1) && !$item->is_ready && ($bag_id > 1 || ($bag_id <= 1 && auth()->guard('worker')->user()->warehouse->auto_print_pp_invoice))) {
                    return response()->json([
