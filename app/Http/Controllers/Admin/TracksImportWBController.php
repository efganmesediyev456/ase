...
class TracksImportWBController extends Controller
{
       protected $view = [
           'name' => 'Import Wildberry Tracks',
	   'mod_name' => 'tracks_import_wb',
       ];
       protected $list = [
        'num'          => [
            'label' => '#',
...
/*        'zip_code'          => [
	...
	if(!empty($parcel_name)) {
	    $container=Container::where('name',$parcel_name)->where('partner_id',2)->first();
	    if(!$container) {
	        $container=new Container();
		$container->name=$parcel_name;
		$container->partner_id=2;
		$container->save();
	    }
	...
	foreach($arr as $title) {
		...
		case 'FIN CODE' :
		    $this->titles['fin']=$cnt;
		    break;
		case 'Адрес пунткта выдачи WB' :
		    $this->titles['access_point']=$cnt;
		    break;
	    }
	    $cnt++;
	}
	...
	$track=Track::where('tracking_code',$arr[$this->titles['tracking_code']])->where('partner_id',2)->first();
	$track_new=false;
	if(!$track) {
	    $track=new Track();
	    $track_new=true;
	}
	$track->partner_id=2;//wildberry
	...
	$track->fin=$arr[$this->titles['fin']];
	if(array_key_exists('access_point',$this->titles))
	    $track->address=$arr[$this->titles['access_point']];
