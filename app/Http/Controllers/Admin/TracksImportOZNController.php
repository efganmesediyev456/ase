...
class TracksImportOZNController extends Controller
{
       protected $view = [
           'name' => 'Import Ozon Tracks',
	   'mod_name' => 'tracks_import_ozn',
       ];
       protected $list = [
        'num'          => [
            'label' => '#',
...
/*        'zip_code'          => [
	...
	if(!empty($parcel_name)) {
	    $container=Container::where('name',$parcel_name)->where('partner_id',3)->first();
	    if(!$container) {
	        $container=new Container();
		$container->name=$parcel_name;
		$container->partner_id=3;
		$container->save();
	    }
	...
	$track=Track::where('tracking_code',$arr[$this->titles['tracking_code']])->where('partner_id',3)->first();
	$track_new=false;
	if(!$track) {
	    $track=new Track();
	    $track_new=true;
	}
	$track->partner_id=3;//Ozon
