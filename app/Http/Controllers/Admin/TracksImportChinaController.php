...
class TracksImportChinaController extends Controller
{
       protected $view = [
           'name' => 'Import China Meest Tracks',
	   'mod_name' => 'tracks_import_china',
       ];
       protected $list = [
        'num'          => [
            'label' => '#',
	...
	if(!empty($parcel_name)) {
	    $container=Container::where('name',$parcel_name)->where('partner_id',7)->first();
	    if(!$container) {
	        $container=new Container();
		$container->name=$parcel_name;
		$container->partner_id=7;
		$container->save();
	    }
	...
	if(strpos($arr[$this->titles['tracking_code']],'AZ') !== 0) return NULL;
	$track=Track::where('tracking_code',$arr[$this->titles['tracking_code']])->where('partner_id',7)->first();
	$track_new=false;
	if(!$track) {
	    $track=new Track();
	    $track_new=true;
	}
	$track->partner_id=7;//China meest
