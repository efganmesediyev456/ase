...
//        }
        ...
        if ($package) {
            ...
            if ((optional($admin->role)->id == 10 && $package->in_customs_status == 1) or (optional($admin->role)->id == 26 && $package->in_customs_status == 1)) {
                $package->scanned_at = now();
                $package->save();
                return response()->json([
                    'error' => "Bu bağlama SAXLANC hesabındadır. " . "Səbəb - " . $package->worker_comments
                ]);
            }
