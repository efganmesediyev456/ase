...
class TracksImportCSERUController extends Controller
{
       protected $view = [
           'name' => 'Import CSERU Tracks',
	   'mod_name' => 'tracks_import_cseru',
       ];
       protected $list = [
        'num'          => [
            'label' => '#',
	...
	if(!empty($parcel_name)) {
	    $container=Container::where('name',$parcel_name)->where('partner_id',5)->first();
	    if(!$container) {
	        $container=new Container();
		$container->name=$parcel_name;
		$container->partner_id=5;
		$container->save();
	    }
	...
	foreach($arr as $title) {
		...
		case 'Consignee name' :
		    ...
		    $titles_cnt++;
		    break;
		case 'Consignee adress' :
		    $this->titles['street']=$cnt;
		    $titles_cnt++;
		    break;
		case 'Consignee phone contact number' :
	...
	$track=Track::where('tracking_code',$arr[$this->titles['tracking_code']])->where('partner_id',5)->first();
	$track_new=false;
	if(!$track) {
	    $track=new Track();
	    $track_new=true;
	}
	$track->partner_id=5;//CSE RU
