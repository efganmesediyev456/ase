...
class TracksImportTMUAZController extends Controller
{
       protected $view = [
           'name' => 'Import TMUAZ Tracks',
	   'mod_name' => 'tracks_import_tmuaz',
       ];
       protected $list = [
        'num'          => [
            'label' => '#',
	...
	if(!empty($parcel_name)) {
	    $container=Container::where('name',$parcel_name)->where('partner_id',4)->first();
	    if(!$container) {
	        $container=new Container();
		$container->name=$parcel_name;
		$container->partner_id=4;
		$container->save();
	    }
	...
	foreach($arr as $title) {
		...
		case 'HAWB Number' :
		    ...
		    $titles_cnt++;
		    break;
		case 'Consignee First name CYR':
		    $this->titles['name']=$cnt;
		    $titles_cnt++;
		    break;
		case 'Consignee Family Name CYR':
		    $this->titles['surname']=$cnt;
		    $titles_cnt++;
		    break;
		case 'Consignee full address CYR':
		    $this->titles['address']=$cnt;
		    $titles_cnt++;
		    break;
		case 'Consignee e-mail addres':
		    $this->titles['email']=$cnt;
		    $titles_cnt++;
		    break;
		case 'Total Parcel Weight, kg brutto' :
		case 'Item total weight' :
		...
		case 'Building' :
		    ...
		    break;
		case 'Phone' :
		case 'Consignee Mobile/phone number' :
		...
		case 'ItemsDescrRU' :
		    ...
		    break;
		case 'Item Discription CYR' :
		case 'ItemsDescrEN' :
		    $this->titles['items_descr_en']=$cnt;
		    $titles_cnt++;
		    break;
		case 'Items Quantity' :
		case 'Number of Item pieces' :
	...
	if(strpos($arr[$this->titles['tracking_code']],'TMUAZ') !== 0) return NULL;
	$track=Track::where('tracking_code',$arr[$this->titles['tracking_code']])->where('partner_id',4)->first();
	$track_new=false;
	if(!$track) {
	    $track=new Track();
	    $track_new=true;
	}
	$track->partner_id=4;//TMUAZ
	$track->container_id=$container_id;
	$track->tracking_code=$arr[$this->titles['tracking_code']];
	$track->weight=str_replace(',','.',$arr[$this->titles['total_weight']]);
	if(array_key_exists('full_name',$this->titles))
	  $track->fullname=$arr[$this->titles['full_name']];
	else
	  $track->fullname=$arr[$this->titles['name']].' '.$arr[$this->titles['surname']];
	...
	if(array_key_exists('region',$this->titles))
	    $track->region_name=$arr[$this->titles['region']];
	if(array_key_exists('city',$this->titles))
	    $track->city_name=$arr[$this->titles['city']];
	if(array_key_exists('address',$this->titles))
	    $track->address=$arr[$this->titles['address']];
	else {
	...
	$str=str_replace('.00','',$str);
	$track->phone=$str;
	if(array_key_exists('currency',$this->titles))
	    $track->currency=$arr[$this->titles['currency']];
	$track->shipping_amount=str_replace(',','.',$arr[$this->titles['value']]);
