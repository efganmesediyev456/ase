...
class PaymentError extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'payment:error';
    ...
    {
        $transactions = Transaction::whereType('ERROR')->where('request_all','!=',null)->where('paid_by', 'PORTMANAT')->latest()->get();

        foreach ($transactions as $transaction) {
            //$data = \GuzzleHttp\json_decode($transaction->request_all, true);
            $rrn = $transaction->source_id;
            ...
            if ($package && !$package->paid) {
                ...
                if ($obj['code'] == '0') {
                    ...
                    $transaction->request_all = json_encode($obj, true);
                    $transaction->save();
